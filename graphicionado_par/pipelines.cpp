...
void initPipelines(unsigned int numEdges) {
	...
	for(unsigned int l = 0; l < NUM_STREAMS; ++l){
		...
		for(unsigned int stream = 0; stream < NUM_STREAMS; ++stream){
			localQueue[l][stream] = new DataCrossbar[queueSizes[l][stream]];
		}
	}
	...
	for(unsigned int i = 0; i < NUM_STREAMS; ++i){
		int streamQueueSize = 0;
		for(unsigned int j = 0; j < NUM_STREAMS; ++j) {
			streamQueueSize += queueSizes[j][i];
		}
		outputQueue[i] = argo::conew_array<DataCrossbar>(streamQueueSize);
	}
...
void mergeQueues(unsigned int ID){
	...
	for(unsigned int i = 0; i < NUM_STREAMS; ++i) {
		for(unsigned int j = 0; j < localCounter[ID][i]; ++j){
			unsigned int posCount = outputCount[i] + j;
			outputQueue[i][posCount] = localQueue[ID][i][j]; //outputCount where to put it in, where j is the fill in from 
		}
		outputCount[i] += localCounter[ID][i];
	}
...
void processingPhaseDestinationOriented(unsigned int ID){
	...
	outputCount[ID] = 0; // reset output queue counter for the next iteration
